...
num_trials=10
playtimes = list(range(200,4200,200)) #[200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000] #, 400, 600, 800, 1000]  # list(range(0,2200,200))
...
def build_p():
    ...
    for k in trials:
        for p in playtimes:
            ...
            playf = f"{ROOT_FILE}/refactored_programs_pncorrect/strings/{SETUP}/refactored_programs-{p}-{k}.pl" # "'data/play-{}-{}'".format(p,k)
...
def test_new():
    ...
    for k in trials:
        for p in playtimes:
            ...
            resultsf = f'./results_pncorrect/{SETUP}/programs-{p}-{k}.pl'
            expf = f"'./programs_pncorrect/{SETUP}/experiment-{p}-{k}.pl'"  # f"programs/{s}/programs-{p}-{k}.pl"
            #call_prolog('do_test',[buildf,programf],resultsf)
...
def get_acc_new(setup,p):
    ...
    for k in trials:
        k_acc=[]
        fname = f'results_pncorrect/{setup}/programs-{p}-{k}.pl'
        with open(fname,'r') as f:
            data=f.read()
            probs=data.split('%')
            for prob in probs:
                ...
                for vs in xs[1:]:
                    ...
                    all_accs.append(int(vs[1]))
                    k_acc.append(int(vs[1]))
        print({"system": "knorf", "playtask": p, "trial": k, "accuracy": np.mean(k_acc)})
        accs.append(np.mean(k_acc))
    return (np.mean(accs),stats.sem(accs),all_accs)
...
def parse_size(setup, p, k):
    playf = f"{ROOT_FILE}/refactored_programs_pncorrect/strings/{setup}/refactored_programs-{p}-{k}.pl.log"
    last_non_empty_line = ""
    fil = open(playf)
