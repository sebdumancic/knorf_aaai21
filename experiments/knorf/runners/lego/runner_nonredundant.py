...
def build_p():
    ...
    if not os.path.exists("./programs_nonredundant"):
    ...
    for k in trials:
        for p in playtimes:
            ...
            playf = f"{ROOT_FILE}/nonredundant_programs/lego/refactored_programs-{p}-{k}.pl" # "'data/play-{}-{}'".format(p,k)
...
def test_new():
    ...
    for k in trials:
        for p in playtimes:
            ...
            resultsf = f'./results_nonredundant/{p}-{k}.pl'
            expf = f"'./programs_nonredundant/experiment-{p}-{k}.pl'"  # f"programs/{s}/programs-{p}-{k}.pl"
            #call_prolog('do_test',[buildf,programf],resultsf)
...
def get_accs_new(setup, p):
    ...
    for k in trials:
        k_num_solved=[]
        fname = f'./results_nonredundant/{p}-{k}.pl'
        with open(fname,'r') as f:
            for line in f:
                line=line.strip()
                xs=line.split(',')
